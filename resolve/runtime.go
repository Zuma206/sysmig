...
func resolve(oldStateJson string) *Resolution {
	...
	stdlib.PushMigratorFunc(state, -1)
...
func getResolution(state *lua.State, index int) *Resolution {
	stdlib.PushResolutionNextState(state, index)
	...
	return &Resolution{
		migrationScript: stdlib.GetResolutionMigration(state, index),
		syncScript:      stdlib.GetResolutionSync(state, index),
